...
void INIT_KEYPAD(){
	...
	KEYPAD_PORT_DIR &= ~( KEY_PAD_IC_MASK ); //set mask pins as input
	#else // use port pins upper or lower
	#if (1 == KEYPAD_UPPER_PORT_PINS)
	KEYPAD_PORT_DIR &= ~( 0xf0 );
	#else
	KEYPAD_PORT_DIR &= ~( 0x0f);
	#endif // end if of select upper or lower
	#endif // end keypad mask if
}
...
uint8_t KeyPad_getPressedKey(void){
	...
	#endif  // end keypad mask use normal upper pins
	#if ( 1 == KEYPAD_UPPER_PORT_PINS)
	//setting the upper pin port as input
	//KEYPAD_PORT_DIR &= ~( 0xf0 );  moved to init_keypad
